...
from keras.layers import Conv2D, BatchNormalization, Activation, MaxPooling2D, AveragePooling2D, add

def Conv2d_BN_Relu(x, filters, kernel_size, name, strides=(1,1), padding='same'):  
    x = Conv2D(filters,kernel_size,padding=padding,strides=strides,name=name+'_conv')(x)  
    x = BatchNormalization(axis=3,name=name+'_bn')(x)  
    x = Activation('relu')(x)
    return x  

def Conv2d_BN(x, filters, kernel_size, name, strides=(1,1), padding='same'):  
    x = Conv2D(filters,kernel_size,padding=padding,strides=strides,activation='relu',name=name+'_conv')(x)  
    x = BatchNormalization(axis=3,name=name+'_bn')(x)  
    return x  
  
def Conv_Block(inpt,filters, kernel_size, name, strides=(1,1), shortcut=False):  
    x = Conv2d_BN(inpt,filters=filters,kernel_size=kernel_size,name=name+'a',strides=strides,padding='same')  
    x = Conv2d_BN(x, filters=filters, kernel_size=kernel_size,name=name+'b',padding='same')  
    if shortcut:  
        shortcut = Conv2d_BN(inpt,filters=filters,kernel_size=kernel_size,name=name+'s',strides=strides)  
        x = add([x,shortcut])  
        return x  
    else:  
        x = add([x,inpt])  
...
class VelocityNet():
    def __init__(self, im_shape):
        ...
        x = Conv2d_BN(img,filters=64,kernel_size=(7,7),name='00',strides=(2,2),padding='same')  
        ...
        # ResNet34: 128
        x = Conv_Block(x,filters=128,kernel_size=(3,3),name='20',strides=(2,2),shortcut=True)  
        ...
        # ResNet34: 256 
        x = Conv_Block(x,filters=256,kernel_size=(3,3),name='30',strides=(2,2),shortcut=True)  
        ...
        # ResNet34: 512 
        x = Conv_Block(x,filters=512,kernel_size=(3,3),name='40',strides=(2,2),shortcut=True)  
        ...
        x = Conv2D(6,kernel_size=(1,1),name='V5')(x)
    
        vel = AveragePooling2D(pool_size=(im_shape[0]/32, im_shape[1]/32))(x)
