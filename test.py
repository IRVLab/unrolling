...
def getGS2RSFlow(depth_rs, cam1, anchors_t_r):
...
def rectifyImgByFlow(img_input, flow_pred):
    img = cv2.cvtColor(np.uint8(img_input*255), cv2.COLOR_GRAY2RGB)
    h, w = img.shape[:2]
...
if num_anchor == 0:
    ...
    ), 'model/checkpoints/model_unroll.hdf5'))
elif num_anchor > 0:
    ...
    ), 'model/checkpoints/model_anchor{}.hdf5'.format(num_anchor)))

# path to save results
save_dir = os.path.join(
    os.getcwd(), "test_results/{}/".format(num_anchor))
...
print('Improved Ratio: {}'.format(win_dv/len(err_dv)))
